...
public class JacobiClothPBD_CPU : MonoBehaviour
{
	...
	void Strain_Limiting()
	{
		...
		Vector3[] sum_x = new Vector3[vertLength];
		float[] sum_n = new float[vertLength];
	...
	void Collision_Handling()
	{
		...
		{
			Vector3 center = spheres[j].transform.position;

			float radius = spheres[j].transform.localScale.x*0.5f+0.2f;
			for (int i = 0; i < X.Length; i++)
			{
				//if (i == 0 || i == 20) continue;
